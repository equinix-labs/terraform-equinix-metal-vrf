...
resource "null_resource" "configure-server-network" {
    ...
    provisioner "remote-exec" {
      inline = [
        ...
        "/root/network-configurator.script ${equinix_metal_vlan.metro_vlan[0].vxlan}",
...
# create a VRF in the same project where the nodes and VLANs are deployed or created and specify the subnet(s) you want 
# to advertise via the VRF (for example "192.168.100.0/24", "192.168.200.0/24")
